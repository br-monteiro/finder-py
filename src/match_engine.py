...
from src.messenger import message, print_matches, show_metrics, help_
from src.disc_manager import is_directory, isfile, get_listdir, loadfile
...
def discoverer(regex, path, level=0, metric: Metric = None):
    ...
    if is_directory(path):
        ...
        for current_path in get_listdir(path):
            ...
            elif is_directory(current_path) and params.isrecursive() and level < recursive_level:
...
def run():
    ...
    if term:
        ...
        discoverer(regex_term, params.get_path(), 0, metric)
        show_metrics(metric)
